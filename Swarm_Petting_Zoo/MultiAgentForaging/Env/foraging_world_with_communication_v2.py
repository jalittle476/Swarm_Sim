        ...
        self.observation_space = spaces.Dict(
            {
                ...
                "money": spaces.Box(0, size - 1, shape=(2,), dtype=int),
            }
        )
...
# Extending the ForagingEnvironment to include communication capabilities
class ForagingEnvironmentWithCommunication(ForagingEnvironment):
    def __init__(self, num_agents, render_mode=None, size=20, seed=255, num_resources=5, fov=2, show_fov=False, show_gridlines=False, draw_numbers=False, record_sim=False):
        super().__init__(num_agents, render_mode, size, seed, num_resources, fov, show_fov, show_gridlines, draw_numbers, record_sim)
        
        # Modify the observation space to include a communication space after it has been initialized
        self.observation_space = spaces.Dict({
            **self.observation_space.spaces, 
            "messages": spaces.Box(low=0, high=255, shape=(self.num_agents, 10), dtype=np.uint8)  # Simplified message space
        })

        # Ensure that the terminated and truncated attributes are properly initialized
    ...
    def observe(self, agent):
        ...
        messages = np.zeros((self.num_agents, 10), dtype=np.uint8)  # Placeholder for messages
        if self.infos[agent]["message"] is not None:
            sender_idx = int(agent.split("_")[1])
            messages[sender_idx] = self.infos[agent]["message"]
        return {**observation, "messages": messages}

    def step(self, action):
        super().step(action)
        
        # Simulate sending a message (this logic can be adjusted as needed)
        for agent in self.agents:
            if np.random.rand() < 0.1:  # 10% chance to send a message
                message = self.np_random.integers(0, 255, size=10, dtype=np.uint8)  # Random message
                self.infos[agent]["message"] = message
        
        # Broadcasting the message to all other agents
        for agent in self.agents:
            if self.infos[agent]["message"] is not None:
                for recipient in self.agents:
                    if recipient != agent:
                        self.infos[recipient]["message"] = self.infos[agent]["message"]

    def reset(self):
        super().reset()
        for agent in self.agents:
            self.infos[agent] = {"message": None}
