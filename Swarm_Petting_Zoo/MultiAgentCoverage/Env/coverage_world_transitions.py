...
class CoverageEnvironment(AECEnv):
    ...
    #         # Check if movement is successful
    #         if np.random.rand() < movement_success_probability:
    #             new_location = intended_location
    #             moved = True
    #         else:
    #             new_location = self._agent_locations[agent]  # Agent stays in place if movement fails
    ...
    #             sensor_success = np.random.rand() < sensor_success_probability
                
    #             # Calculate reward based on sensor success and discovery
    #             current_reward = self.calculate_mapping_reward(sensor_success, visits, total_visits)
    #             self.reward_grid[new_location[0], new_location[1]] = current_reward
    #             self.coverage_grid[new_location[0], new_location[1]] += 1 if sensor_success else 0
